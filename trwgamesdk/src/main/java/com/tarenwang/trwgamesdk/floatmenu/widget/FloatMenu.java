...
public class FloatMenu extends FrameLayout implements View.OnTouchListener {
                ...
                if (isInitingLoader) {
                    ...
                    if (mIsRight) {
                        if (params.rightMargin <= 0) {
                            mFloatLogoFra.setPadding(16, 0, 0, 16);
                        ...
                        if (params.leftMargin >= 0) {
                            ...
                            mFloatLogoFra.setLayoutParams(params);
                            mFloatLogoFra.setPadding(0, 0, 16, 16);
                        }
                    }
    ...
    private void resetLogoSize() {
        LayoutParams floatLogoLayoutParams = FloatMenuUtils.createLayoutParams(FloatMenuUtils.dp2Px(50, SDKHelper.getmInstance().getContext()),FloatMenuUtils.dp2Px(50, SDKHelper.getmInstance().getContext()));
    ...
    private void init(Context context) {
        ...
        //默认指定的纵坐标为屏幕高度的二分之一，这里只是大概约束，因为上的flags参数限制，悬浮球不会出现在屏幕外
...
//        addView();
    ...
    private View createView( final Context mContext) {
        View rootView = View.inflate(mContext, ResourceUtils.getLayoutId(SDKHelper.getmInstance().getContext(),"trw_float_menu"),null);//将xml布局导入当前view
        FrameLayout rootFloatView = (FrameLayout) rootView.findViewById(ResourceUtils.getId(SDKHelper.getmInstance().getContext(),"trwGameMenu"));
        mFloatMenuLine= (LinearLayout) rootView.findViewById(ResourceUtils.getId(SDKHelper.getmInstance().getContext(),"trwGameMenuLine"));
        mFloatLogoFra = (FrameLayout) rootView.findViewById(ResourceUtils.getId(SDKHelper.getmInstance().getContext(),"trwGameFra"));
        mFloatLogoImv = (ImageView) rootView.findViewById(ResourceUtils.getId(SDKHelper.getmInstance().getContext(),"trwGameLogo"));
        mFloatLoaderImv = (ImageView) rootView.findViewById(ResourceUtils.getId(SDKHelper.getmInstance().getContext(),"trwGameLoader"));
        ...
        //设置当前悬浮球的touchListener事件，此listener设置后 ，ontouchevent方法失效
    ...
    private void refreshFloatMenu(boolean isRight) {
        int padding = FloatMenuUtils.dp2Px(4, mContext);
        int padding52 = FloatMenuUtils.dp2Px(50, mContext);
